...
const ASSET_TYPE_DEFS: { value: BrandAsset['type']; labelKey: string; icon: string }[] = [
  { value: 'product_photo', labelKey: 'assets.types.product_photo', icon: 'inventory_2'     },
  { value: 'label',         labelKey: 'assets.types.label',         icon: 'sell'            },
  { value: 'logo',          labelKey: 'assets.types.logo',          icon: 'brand_awareness' },
  { value: 'photo',         labelKey: 'assets.types.photo',         icon: 'landscape'       },
  { value: 'screenshot',    labelKey: 'assets.types.screenshot',    icon: 'computer'        },
  { value: 'other',         labelKey: 'assets.types.other',         icon: 'category'        },
]
...
function TypeBadge({ asset, onChange }: { asset: BrandAsset; onChange: (type: BrandAsset['type']) => void }) {
  ...
  const { t } = useLanguage()
  const ASSET_TYPES = ASSET_TYPE_DEFS.map(d => ({ ...d, label: t(d.labelKey) }))
  const current = ASSET_TYPES.find(a => a.value === asset.type) ?? ASSET_TYPES[4]
        ...
        <>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: GOLD, fontFamily: 'var(--font-ibm)', letterSpacing: '.05em', textTransform: 'uppercase' }}>{t('assets.aiDescription')}</span>
            </div>
            <textarea
              value={value}
              onChange={e => setValue(e.target.value)}
              placeholder={t('assets.aiDescPlaceholder')}
              autoFocus
              rows={4}
              style={{
...
export function AssetUploader({ assets, userId, onAssetsChange }: Props) {
  ...
  async function uploadFile(file: File) {
    ...
    try {
      ...
      toast.success(t('assets.toastUploaded'))
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : t('assets.toastUploadFailed'))
    } finally {
      setUploading(false)
    }
  }
  ...
  async function deleteAsset(asset: BrandAsset) {
    ...
    if (res.ok) { onAssetsChange(assets.filter(a => a.id !== asset.id)); toast.success(t('assets.toastDeleted')) }
    else toast.error(t('assets.toastDeleteFailed'))
  }
  ...
  async function updateDescription(asset: BrandAsset, description: string) {
    ...
    if (res.ok) {
      ...
      toast.success(t('assets.toastDescSaved'))
    } else {
      toast.error(t('assets.toastDescFailed'))
    }
  }
      ...
      >
        ...
        {uploading ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12 }}>
            <Spinner />
            <span style={{ color: 'var(--sand)', fontSize: 14 }}>{t('common.uploading')}</span>
          </div>
        ) : (
          <>
            <Upload size={28} style={{ color: 'var(--sand)', margin: '0 auto 12px' }} />
            <p style={{ color: 'var(--sand)', fontSize: 14, fontWeight: 300 }}>
              {t('assets.uploadDrop')} <span style={{ color: 'var(--candle)' }}>{t('assets.uploadBrowse')}</span>
            </p>
            <p style={{ color: 'var(--muted)', fontSize: 12, marginTop: 4 }}>{t('assets.uploadFormats')}</p>
          </>
        )}
      </div>
